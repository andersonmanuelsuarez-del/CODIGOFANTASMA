...
namespace NinjaTrader.NinjaScript.Strategies
{
    ...
    public class Anza : Strategy
    {
        ...
        // ── Control de Balance Objetivo ───────────────────────
        private bool balanceObjetivoAlcanzado = false;
        ...
        // ─── 5) Control de Balance para Fondeo ───
        [NinjaScriptProperty]
        [Display(Name = "Activar control de balance objetivo", GroupName = "4) Control de Balance (Fondeo)", Order = 1,
            Description = "Cuando se activa, la estrategia cierra todas las posiciones y detiene operaciones al alcanzar el balance objetivo.")]
        public bool UsarControlBalance { get; set; }
        [NinjaScriptProperty]
        [Range(0, 100000000)]
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (Bars.IsFirstBarOfSession)
            {
                ...
                // Reset del control de balance al inicio de cada sesión
                balanceObjetivoAlcanzado = false;
            ...
            // ══════════════════════════════════════════════════
            //  CONTROL DE BALANCE OBJETIVO (fondeo)
            // ──────────────────────────────────────────────────
            //  Verifica si el equity total de la cuenta ha
            //  alcanzado el objetivo. Si es así, cierra todo
            //  y bloquea nuevas entradas por el resto del día.
            // ══════════════════════════════════════════════════
            if (UsarControlBalance && !balanceObjetivoAlcanzado)
            {
                double equityActual = ObtenerEquityTotal(Close[0]);
                if (equityActual >= BalanceObjetivo)
                {
                    balanceObjetivoAlcanzado = true;
                    if (Position.MarketPosition == MarketPosition.Long)
                    {
                        ExitLong("Balance Objetivo", "ANZA_Long");
                        Print($"[Anza-BALANCE] {Time[0]} ¡BALANCE OBJETIVO ALCANZADO! Equity: ${equityActual:F2} >= Objetivo: ${BalanceObjetivo:F2}. Posición LONG cerrada. Operaciones detenidas.");
                    }
                    else if (Position.MarketPosition == MarketPosition.Short)
                    {
                        ExitShort("Balance Objetivo", "ANZA_Short");
                        Print($"[Anza-BALANCE] {Time[0]} ¡BALANCE OBJETIVO ALCANZADO! Equity: ${equityActual:F2} >= Objetivo: ${BalanceObjetivo:F2}. Posición SHORT cerrada. Operaciones detenidas.");
                    }
                    else
                    {
                        Print($"[Anza-BALANCE] {Time[0]} ¡BALANCE OBJETIVO ALCANZADO! Equity: ${equityActual:F2} >= Objetivo: ${BalanceObjetivo:F2}. Sin posición abierta. Operaciones detenidas.");
                    }
                    return;
                }
            }
            // Si el objetivo ya fue alcanzado hoy, no hacer nada más
            if (UsarControlBalance && balanceObjetivoAlcanzado)
            {
                return;
            }
        ...
        // ════════════════════════════════════════════════════
        //  MONITOREO EN TIEMPO REAL — TICK A TICK
        // ────────────────────────────────────────────────────
        //  Mientras hay una posición abierta, este método se
        //  ejecuta en cada tick del mercado para verificar si
        //  el equity alcanzó el objetivo. Esto garantiza un
        //  cierre lo más preciso posible sin esperar al cierre
        //  de la barra.
        // ════════════════════════════════════════════════════
        protected override void OnMarketData(MarketDataEventArgs marketDataUpdate)
        {
            ...
            if (!UsarControlBalance || balanceObjetivoAlcanzado)
                return;
            // Solo verificar cuando hay posición abierta (para eficiencia)
            ...
            if (equityActual >= BalanceObjetivo)
            {
                ...
                Print($"[Anza-BALANCE] ¡OBJETIVO ALCANZADO EN TIEMPO REAL! Equity: ${equityActual:F2} >= Objetivo: ${BalanceObjetivo:F2}. Posición cerrada. Sin más operaciones hoy.");
            }
        }
        // ════════════════════════════════════════════════════
        //  CÁLCULO DE EQUITY TOTAL DE LA CUENTA
        // ────────────────────────────────────────────────────
        //  Combina el cash de la cuenta (ganancias realizadas)
        //  con la ganancia/pérdida no realizada de la posición
        //  actual para obtener el equity total en tiempo real.
        // ════════════════════════════════════════════════════
        private double ObtenerEquityTotal(double precioReferencia)
        {
            double cashValue = Account.Get(AccountItem.CashValue, Currency.UsDollar);
            double unrealizedPnL = 0;
            if (Position.MarketPosition != MarketPosition.Flat)
            {
                unrealizedPnL = Position.GetUnrealizedProfitLoss(PerformanceUnit.Currency, precioReferencia);
            }
            return cashValue + unrealizedPnL;
        }
        // ════════════════════════════════════════════════════
            ...
            if (execution.Order != null && execution.Order.OrderState == OrderState.Filled)
            {
                ...
                else if (tradeActivo)
                {
                    ...
                    if (currentPositionQty <= 0)
                    {
                        ...
                        // Log de balance después de cerrar trade (si control activo)
                        if (UsarControlBalance)
                        {
                            double balancePostTrade = Account.Get(AccountItem.CashValue, Currency.UsDollar);
                            Print($"[Anza-BALANCE] Balance post-trade: ${balancePostTrade:F2} | Objetivo: ${BalanceObjetivo:F2} | Diferencia: ${(BalanceObjetivo - balancePostTrade):F2}");
                        }
                        precioEntradaRef = double.NaN;
                        stopLossInicialRef = double.NaN;
                        peorPrecioTrade = double.NaN;
